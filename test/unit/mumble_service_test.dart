...
@GenerateNiceMocks([MockSpec<MumbleClient>(), MockSpec<AudioRecorder>(), MockSpec<AudioClient>()])
  ...
  group('MumbleService Audio Loopback Concept', () {
    test('Encoder and Decoder Loopback produces matching audio samples', () {
       ...
       try {
         ...
         expect(decodedSamples.length, frameSize, reason: 'Decoded size mismatch');
         ...
         expect(matchingSignificantSamples, greaterThan(80), 
           reason: 'Audio quality loss too high or decoding corrupted');

         print('Round trip: Encode=${encodeTime.inMicroseconds}µs, Decode=${decodeTime.inMicroseconds}µs');
         ...
         if (e.toString().contains('Failed to lookup symbol') || 
           ...
           print('Skipping Opus Native test: Opus library not available in current test environment.');
           return;
         }
         rethrow;
       }
